...
3. **Theme access** — All colors must come from the theme via `${({ theme }) => theme.<key>}`. Flag any hardcoded hex colors inside styled-components (exception: the vignette in Skidbladnir uses `#1C1C27` intentionally — ignore that one).
...
5. **Framer Motion** — In `Skidbladnir/index.js` and `UlrichEasterEgg.js`, verify that animated values come from `useTransform` / `useScroll` and are passed via the `style` prop (not via state re-renders). Flag any `useState` used to drive animations that framer-motion could handle natively.
