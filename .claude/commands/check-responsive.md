...
4. **Photography grid** — Verify the grid transitions: 3 columns → 2 columns at ≤960px → 1 column at ≤600px.

5. **Text overflow** — Flag any `Title` or `Desc` styled component that doesn't set `text-align: center` at mobile breakpoints, which can cause text to overflow its container on small screens.
